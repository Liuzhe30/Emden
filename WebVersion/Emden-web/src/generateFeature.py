...
outputfile = open('/data/emden/Emden-web/datasets/middlefile/seq_' + str(request_no) + '_before.fasta', 'w')
w_content = '>seq_' + str(request_no) + '\n' + fasta_full_before
outputfile.write(w_content)
outputfile.close()
outputfile = open('/data/emden/Emden-web/datasets/middlefile/seq_' + str(request_no) + '_after.fasta', 'w')
w_content = '>seq_' + str(request_no) + '\n' + fasta_full_after
outputfile.write(w_content)
outputfile.close()

os.system('hhblits -i /data/emden/Emden-web/datasets/middlefile/seq_' + str(request_no) + '_before.fasta -d /data/emden/uniprot20_2016_02/uniprot20_2016_02 -ohhm /data/emden/Emden-web/datasets/middlefile/hhm/' + str(request_no) + '_before.hhm')
os.system('hhblits -i /data/emden/Emden-web/datasets/middlefile/seq_' + str(request_no) + '_after.fasta -d /data/emden/uniprot20_2016_02/uniprot20_2016_02 -ohhm /data/emden/Emden-web/datasets/middlefile/hhm/' + str(request_no) + '_after.hhm')
...
civic_drug_table = pd.read_pickle('/data/emden/Emden-web/datasets/middlefile/civic_drug_table_fpfixed_smilenum' + str(request_no) + '.pkl')
...
# hhm before
with open('/data/emden/Emden-web/datasets/middlefile/hhm/' + str(request_no) + '_before.hhm') as hhm_file:
...
# hhm after
with open('/data/emden/Emden-web/datasets/middlefile/hhm/' + str(request_no) + '_after.hhm') as hhm_file:
    hhm_matrix = np.zeros([fasta_len, 30], float)
    hhm_line = hhm_file.readline()
    idxx = 0
    while(hhm_line[0] != '#'):
...
filtered_dataset_feature.to_pickle('/data/emden/Emden-web/datasets/dataset_featurecode' + str(request_no) + '.dataset')